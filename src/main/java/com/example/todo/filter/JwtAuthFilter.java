...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        try {
            ...
            //토큰 위조 검사 및 인증 완료 처리
            if(token != null){
                //토큰 서명 위조 검사와 토큰을 파싱해서 클래임을 얻어내는 작업
                TokenUserInfo userInfo
                        = tokenProvider.validateAndGetTokenUserInfo(token);



                //인가 정보 list(권한이 여러개 존재할 경우 리스트로 권한 체크에 사용할 필드를 add)
                //우리는 Role타입의 필드 하나만으로 권한을 체크하기 때문에 하나만 add
                List<SimpleGrantedAuthority> authorityList = new ArrayList<>();
                authorityList.add(new SimpleGrantedAuthority("ROLE_" + userInfo.getRole().toString()));

                //인증 완료 처리
                //(스프링 시큐리터에게 인증정보를 전달해서 전역적으로 어플리케이션에서
                //인증 정보를 활용할 수 있게 설정)
                ...
                //인증 완료 처리시 클라이언트의 요청 정보 세팅..다음에 인증정보 활용을 위해
